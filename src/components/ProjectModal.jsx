...
import { X, Shield, Activity, Cpu, CheckCircle, Trophy, ExternalLink, Terminal } from 'lucide-react';
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 lg:p-8 bg-black/80 backdrop-blur-xl">
        ...
        <motion.div
          ...
          className={`relative z-10 w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-[#0b0f17] border rounded-2xl p-6 sm:p-8 ${getZoneThemeBorder()} hud-card`}
        >
          {/* Top Bar Header */}
          <div className="flex items-start justify-between gap-4 pb-6 border-b border-white/10">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span className={`font-mono text-xs px-2.5 py-0.5 rounded border uppercase tracking-wider ${getZoneBadgeColor()}`}>
                  {selectedProject.zoneTitle}
                </span>
                {selectedProject.badge && (
                  <span className="font-mono text-xs px-2.5 py-0.5 rounded bg-white/5 border border-white/10 text-white font-bold flex items-center gap-1.5">
                    {selectedProject.badge.includes('WINNER') && <Trophy className="w-3.5 h-3.5 text-amber-400" />}
              ...
              <div className="font-mono text-xs sm:text-sm text-slate-300 mt-1">
          ...
          <div className="py-6 space-y-6">
            ...
            {/* Overview Section */}
            <div>
              <h4 className="font-mono text-xs text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1.5 font-semibold">
            ...
            {selectedProject.telemetry && (
              <div className="bg-black/50 p-4 rounded-xl border border-white/10">
                <h4 className="font-mono text-xs text-slate-400 uppercase tracking-wider mb-3 flex items-center gap-1.5 font-semibold">
                  <Activity className="w-4 h-4 text-cyan-400" />
                  <span>MEASURED OPERATIONAL TELEMETRY:</span>
                </h4>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {Object.entries(selectedProject.telemetry).map(([key, value]) => (
                    <div key={key} className="p-3 rounded-lg bg-white/5 border border-white/5 font-mono">
                      <div className="text-[10px] text-slate-400 uppercase tracking-wider">
                        {key.replace(/([A-Z])/g, ' $1')}
                      </div>
                      <div className="text-xs sm:text-sm text-white font-bold mt-1">
                        {value}
                      </div>
                    </div>
                  ))}
            ...
            <div>
              ...
              <div className="space-y-2">
                {selectedProject.highlights.map((highlight, i) => (
                  <div
                    key={`modal-high-${i}`}
                    className="p-3 rounded-lg bg-white/5 border border-white/5 text-xs sm:text-sm text-slate-200 flex items-start gap-2.5"
                  >
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {selectedProject.techStack.map((tech, i) => (
                  <span
                    key={`modal-tech-${i}`}
                    className="font-mono text-xs px-3 py-1 rounded-lg bg-white/5 border border-white/10 text-slate-200"
                  >
                    {tech}
                  </span>
                ))}
          ...
          <div className="pt-6 border-t border-white/10 flex items-center justify-between">
            ...
            <button
              onClick={closeProjectModal}
              onMouseEnter={() => triggerSound('hover')}
              className="px-5 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-white font-mono text-xs tracking-wider uppercase transition-all cursor-pointer"
            >
              CLOSE TELEMETRY HUD
            </button>
          </div>
